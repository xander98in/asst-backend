...
public class BatteryManagementRecordSpecification {
    ...
    /**
     * Construye una Specification dinámica para filtrar baterías que pertenecen a un espacio
     * de análisis específico, combinando múltiples criterios opcionales.
     *
     * <p>A diferencia de {@link #withClosedStatusAndFilters}, no re-valida el estado "Cerrado"
     * porque esa regla ya se impone al momento de agregar la batería al espacio.</p>
     *
     ...
     * @return Specification con los filtros aplicados
     */
    public static Specification<BatteryManagementRecordEntity> withSpaceAndFilters(
        Long spaceId,
        String identificationNumber,
        String workAreaName,
        LocalDateTime dateFrom,
        ...
        return (root, query, cb) -> {
            ...
            // Filtro obligatorio: la batería debe pertenecer al espacio de análisis.
            Subquery<Long> spaceSubquery = query.subquery(Long.class);
            Root<AnalysisSpaceBatteryEntity> spaceBatteryRoot =
                spaceSubquery.from(AnalysisSpaceBatteryEntity.class);
            spaceSubquery.select(cb.literal(1L))
                .where(cb.and(
                    cb.equal(spaceBatteryRoot.get("analysisSpace").get("id"), spaceId),
                    cb.equal(spaceBatteryRoot.get("batteryManagementRecordId"), root.get("id"))
                ));
            predicates.add(cb.exists(spaceSubquery));
